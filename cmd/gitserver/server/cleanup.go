...
func (s *Server) cleanupRepos() {
	...
	maybeRemoveCorrupt := func(dir GitDir) (done bool, _ error) {
		...
		s.Log.Info("removing corrupt repo", log.String("repo", string(dir)), log.String("reason", reason))
	...
	maybeReclone := func(dir GitDir) (done bool, err error) {
		...
		repo := s.name(dir)
		s.Log.Info("re-cloning expired repo", log.String("repo", string(repo)), log.Time("cloned", recloneTime), log.String("reason", reason))
	...
	err := bestEffortWalk(s.ReposDir, func(dir string, fi fs.FileInfo) error {
		...
		for _, cfn := range cleanups {
			...
			if err != nil {
				s.Log.Error("error running cleanup command", log.String("name", cfn.Name), log.String("repo", string(gitDir)), log.Error(err))
			}
...
func (s *Server) setRepoSizes(ctx context.Context, repoToSize map[api.RepoName]int64) error {
	...
	s.Log.With(log.Int("repoToSize", len(repoToSize))).Info("cleanup: directory sizes calculated during file system walk")
	...
	s.Log.With(log.Int("reposToUpdate", len(reposToUpdate))).Info("cleanup: repos had their sizes updated")
	return nil
}
